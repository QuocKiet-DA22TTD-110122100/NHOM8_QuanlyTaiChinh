...
function Reports() {
  ...
  const exportReport = () => {
    // Tạo dữ liệu CSV với thông tin chi tiết và dễ đọc
    const csvData = [
      // Header với thông tin tổng quan
      ['BÁO CÁO TÀI CHÍNH NĂM ' + selectedYear],
      ['Ngày tạo: ' + new Date().toLocaleDateString('vi-VN')],
      [''],
      ['TỔNG QUAN'],
      ['Tổng thu nhập:', yearlyStats.totalIncome.toLocaleString('vi-VN') + ' ₫'],
      ['Tổng chi tiêu:', yearlyStats.totalExpense.toLocaleString('vi-VN') + ' ₫'],
      ['Số dư ròng:', yearlyStats.balance.toLocaleString('vi-VN') + ' ₫'],
      ['Tổng giao dịch:', yearlyStats.transactionCount],
      [''],
      ['THU CHI THEO THÁNG'],
      ['Tháng', 'Thu nhập (₫)', 'Chi tiêu (₫)', 'Số dư (₫)'],
      ...
      ...categoryData.map(cat => [
        ...
        ((cat.value / yearlyStats.totalExpense) * 100).toFixed(1) + '%'
      ])
    ];
    ...
    a.href = url;
    a.download = `Bao-cao-tai-chinh-${selectedYear}.csv`;
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button 
            ...
            className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl hover:from-blue-600 hover:to-indigo-700 transition-all duration-200 transform hover:scale-105 shadow-lg"
          >
