...
function Income() {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Quản lý thu nhập</h2>

      {/* Form thêm thu nhập */}
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow">
        <h3 className="text-lg font-medium text-gray-800 dark:text-white mb-4">Thêm khoản thu nhập mới</h3>
      ...
      {/* Bảng danh sách thu nhập */}
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow overflow-x-auto">
        <h3 className="text-lg font-medium text-gray-800 dark:text-white mb-4">Danh sách thu nhập</h3>
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {incomes.map((income) => (
              <tr key={income.id}>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">{income.date}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-white">{income.description}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300">
                  {income.category === 'salary' && 'Lương'}
                  {income.category === 'business' && 'Kinh doanh'}
                  {income.category === 'investment' && 'Đầu tư'}
                  {income.category === 'other' && 'Khác'}
                </td>
